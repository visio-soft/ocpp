...
class ChargePoint(cp):
    ...
    @on(Action.ClearChargingProfile)
    async def on_clear_charging_profile(self, id: int, connector_id: int, charging_profile_purpose: str, stack_level: int):
        return call_result.ClearChargingProfilePayload(
            status=ClearChargingProfileStatus.accepted
        )
    ...
    async def on_update_firmware(self, location: str, retrieve_date, retries: int = None, retry_interval: int = None):
