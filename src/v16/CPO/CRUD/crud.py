...
#Done
async def send_local_list(charge_point_id: str, list_version: int, update_type: str, local_authorization_list: list,
    response_status:str):
...
async def stop_transaction_db(charge_point_id:str, transaction_id: int, id_tag: str, meter_stop:int, timestamp:str, reason:str, transaction_data:list,
    authorization_status:str, expiry_date:str=None, parent_id_tag:str =None):
    timestamp = datetime.strptime(timestamp, '%Y-%m-%dT%H:%M:%S.%f')
    expiry_date = datetime.strptime(expiry_date, '%Y-%m-%dT%H:%M:%S.%f')
...
#Done
async def get_composite_schedule_db(charge_point_id:str, connector_id:int, duration:int, charging_rate_unit:str, 
    schedule_start:str, charging_schedule: dict, response_status:str):
    timestamp=datetime.now().isoformat()
    timestamp = datetime.strptime(timestamp, '%Y-%m-%dT%H:%M:%S.%f')
    schedule_start = datetime.strptime(schedule_start, '%Y-%m-%dT%H:%M:%S.%f')
    start_schedule = datetime.strptime(charging_schedule['start_schedule'], '%Y-%m-%dT%H:%M:%S.%f')
...
async def charging_profile_db(charge_point_id:str, connector_id:int, cs_charging_profiles: dict, response_status:str):
    ...
    timestamp = datetime.strptime(timestamp, '%Y-%m-%dT%H:%M:%S.%f')
    start_schedule = datetime.strptime(charging_schedule.start_schedule, '%Y-%m-%dT%H:%M:%S.%f')
    valid_from = datetime.strptime(cs_charging_profiles.valid_from, '%Y-%m-%dT%H:%M:%S.%f')
    valid_to = datetime.strptime(cs_charging_profiles.valid_to, '%Y-%m-%dT%H:%M:%S.%f')
