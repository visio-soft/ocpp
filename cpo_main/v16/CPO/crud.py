...
async def get_charge_point_sessions(db: Session, charge_point_id: str):
    return db.query(models.ChargePointSessions16).filter(models.ChargePointSessions16.charge_point_id == charge_point_id).all()

async def get_charge_point_sessions_connector(db: Session, charge_point_id: str, connector_id: int = None):
    return db.query(models.ChargePointSessions16).filter(models.ChargePointSessions16.charge_point_id == charge_point_id,
        models.ChargePointSessions16.connector_id == connector_id).all()

async def get_charge_point_session(db: Session, charge_point_id: str, transaction_id):
    return db.query(models.ChargePointSessions16).filter(models.ChargePointSessions16.charge_point_id == charge_point_id, 
        models.ChargePointSessions16.transaction_id == transaction_id).first()
...
async def transaction_db(charge_point_id: str, connector_id: int, id_tag: str, meter_start:int, timestamp:str, reservation_id: str= None):
    transaction = models.ChargePointSessions16(
        charge_point_id=charge_point_id,
        connector_id=connector_id,
        id_tag=id_tag,
    ...
    active_transaction = db.query(models.ChargePointSessions16).filter(models.ChargePointSessions16.charge_point_id == charge_point_id).all()
    if not active_transaction:
        ...
        updated_transaction = db.query(models.ChargePointSessions16).filter(models.ChargePointSessions16.charge_point_id == charge_point_id).update({
            "charge_point_id":charge_point_id,
            "id_tag":id_tag,
            "meter_start":meter_start,
            "timestamp":timestamp,
